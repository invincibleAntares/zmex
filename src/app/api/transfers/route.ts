...
import { transferSchema } from "@/modules/transfers/transfer.schema";
import { executeTransfer } from "@/modules/transfers/transfer.service";
import { checkTransferRateLimit } from "@/modules/transfers/transfer-rate-limit";
import { z } from "zod";
...
export async function POST(request: NextRequest) {
  try {
    ...
    await checkTransferRateLimit(userId);
    ...
    // 4. BODY VALIDATION
    const input = await parseAndValidate(request, transferSchema);
